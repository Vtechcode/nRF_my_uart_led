...
#include <zephyr/drivers/uart.h> //header file of the UART driver
...
/* get device pointer of the uart hardware */
const struct device *uart = DEVICE_DT_GET(DT_NODELABEL(uart0));

/* get device pointer of the gpios */
static const struct gpio_dt_spec led0 = GPIO_DT_SPEC_GET(DT_NODELABEL(my_pin21), gpios);
static const struct gpio_dt_spec led1 = GPIO_DT_SPEC_GET(DT_NODELABEL(my_pin22), gpios);
static const struct gpio_dt_spec led2 = GPIO_DT_SPEC_GET(DT_NODELABEL(my_pin23), gpios);
...
{
 switch (evt->type) {  case UART_RX_RDY:
